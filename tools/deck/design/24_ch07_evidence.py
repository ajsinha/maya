# ============================================================ CH 7
_state["chapter"] = "7 · Evidence, and Six Questions from One Traversal"
...
para(tf, "So the evidence chain — the append-only, hash-linked record of every "
         "act — IS the audit log. The consolidation is load-bearing: "
         "segregation of duties is decided by READING THE CHAIN, which only "
         "works if the chain is the record.",
...
para(tf, "“Has this person already acted incompatibly on this model?” is a "
         ...
         "who-did-what table would answer it from a copy, and the copy goes "
         "stale.",
...
note(sl, ML, y + h + 0.28, CW * 0.54, 1.35,
     ...
     "from a design intention into a theorem the suite runs.")
...
para(tf, "Boolean provenance loses both, so it cannot tell a claim resting on "
         "one document from a claim resting on four — and “how corroborated is "
         "this” is what a reviewer is asking.",
     size=11, color=SLATE, space_after=8, line=1.26)
para(tf, "The same construction annotates DERIVED FEATURES, where the ingest "
         "clock obeys the algebra rather than a rule somebody could forget.",
