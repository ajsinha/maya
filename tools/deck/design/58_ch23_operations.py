...
runs(tf, [("No outbox, no broker, no migrations. ", CRIMSON, True),
          ("No domain event leaves the process; consumers integrate by polling the API. Schema evolution is "
           "expand-and-contract by hand across two files, 46 tables, one database and one flat evidence "
...
data = [["Surface", "What it does"],
        ...
        ["Nine scheduler jobs", "Each idempotent, each re-deriving its own condition; anchoring and full chain verification are two of them"],
        ...
        ["test_laws.py", "Sixteen of the twenty-one foundational laws are executable and run as tests; the five that do not are named with the reason. All fourteen warrant laws run before a signature"]]
...
data = [["Claimed elsewhere", "Here"],
        ...
        ["A build that fails", "There is no CI: no pipeline, no import-linter, no type checker, no linter, no coverage gate. The suite is real and a person runs it"]]
