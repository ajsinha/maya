...
**153 findings, grouped into eleven remaining batches by what has to change rather than by
...
|---|---|---|---|
| **1** | **D** — A read scoped on `?urn=` and unscoped on `/{id}` | 5 | four instances of one leak, one helper closes them |
| **2** | **B** — A control that is built, wired, documented and never fires | 20 | the recurring defect; every fix is connect-or-delete |
| **3** | **A** — A required field accepted blank, absent or impossible | 37 | one pattern, 37 sites, mechanical once the first is written |
| **4** | **F** — Authority bands, delegation, quorum and conditions | 9 | a tier 1 version was approved by one person |
| **5** | **E** — The evidence chain's edges: checkpoint, anchor, timestamp | 13 | what an investigation rests on |
| **6** | **H** — HTTP conventions: paging, projection, headers, refusal shape | 6 | one adoption problem behind six symptoms |
| **7** | **I** — Concurrency, the scheduled batch and delivery | 8 | a half-run batch is worse than no batch |
| **8** | **G** — Documents: compile, dossier, pack, review, search | 20 | the pack is what leaves the building |
| **9** | **K** — Deletion, compaction and the operational tools | 5 | the reclaim path is unreachable today |
| **10** | **L** — Distributed evaluation, monitoring and parameter provenance | 6 | narrower blast radius, needs the most care |
| **11** | **J** — Lifecycle and workflow correctness | 24 | largest, most independent, safest to do last |

---

## 1. Batch D — A read scoped on `?urn=` and unscoped on `/{id}`
...
| Case | What it found |
|---|---|
| `QA-AM-079` | the answer is short by 12 and says nothing about it: no count of what scope removed, and no `detail` saying the total is partial — so a reader reconciling against an estate-wide pack sees… |
...
| `QA-PLT-176` | `search(principal=None)` with an authoriser wired returned 9 result(s) across 13 document(s) — `_visible` applies the filter only `if self.authz is not None and principal is not None`, so… |
...
## 2. Batch B — A control that is built, wired, documented and never fires
...
| `QA-FX-385` | `note_revocation` accepts a `warrant_id`, and the id moves: two resolutions of one grant minted 01a0a2d3a9e36e5b… and 01a0a2d3a9e71b67…, so an id noted from one never matches the one `exe… |
...
## 3. Batch A — A required field accepted blank, absent or impossible
...
| `QA-AM-626` | expected a refusal (overlay_refused/owner_required/validation_error) and the answer carried no error code: 201 {"id":"01a0a2d58c57af018e94d55764af","model_id":"01a0a2d58c3c591233e3355a375… |
| `QA-AM-631` | expected a refusal (validation_error/supervisor_required/matter_refused/regime_refused) and the answer carried no error code: 201 {"id":"01a0a2d58d3be4d86680b144d069","reference":"SUP-b4d… |
| `QA-AM-632` | expected a refusal (validation_error/owner_required/matter_refused/regime_refused) and the answer carried no error code: 201 {"id":"01a0a2d58d6e4888951588c31ce9","reference":"SUP-6d408ceb… |
...
| `QA-FX-800` | expected a refusal (validation_error/parameter_refused/name_required) and the answer carried no error code: 201 {"id":"01a0a2d42a460e6bd52abee709bc","model_id":"01a0a2d42a2cccbc0557498292… |
...
| `QA-FX-954` | expected a refusal (feature_refused/reason_required/validation_error/discovery_refused) and the answer carried no error code: 200 {"id":"01a0a2d42f2f67dc53389336a7e6","view_name":"tv-331d… |
...
| `QA-GOV-514` | expected a refusal (unknown_role/validation_error/authority_refused/lifecycle_refused) and the answer carried no error code: 201 {"id":"01a0a2d321d73a61f00665784d7c","name":"band-4a5587d1… |
...
## 4. Batch F — Authority bands, delegation, quorum and conditions
...
| `QA-GOV-132` | a figure attested as at a date a year away decides today's band: the amount reads 1000.0 and the band is 'bd-ef6eba4d'. `amount_for` takes the greatest `as_at` and nothing refuses one in … |
...
| `QA-GOV-225` | the role 'role-413b4afb' was deleted while a published band names it as a required signature: `_awaited_by_a_quorum` reads the open attestation and version_approval tables only, so the ma… |
...
## 5. Batch E — The evidence chain's edges: checkpoint, anchor, timestamp
...
| `QA-PLT-026` | 1 anchor(s) were deleted from /tmp/maya-qa-tomsmzpu/data/worm and `verify_against_anchors` answers anchored=0, agrees=1: 'no anchors written yet, so the chain is self-certified and nothin… |
| `QA-PLT-027` | the entire anchor root was removed and the next anchor recreated it (/tmp/maya-qa-ypl74u6t/data/worm) and wrote seq 13 into it, reporting written=1. `FilesystemWORM.put` opens with `root.… |
...
## 6. Batch H — HTTP conventions: paging, projection, headers, refusal shape
...
| `QA-PLT-360` | page one and page two share 1 row(s) after one registration between the two reads: ['maya://model/2-rc-64b49e8a']. `/models` pages with LIMIT/OFFSET, so an insert that sorts before the bo… |
...
## 7. Batch I — Concurrency, the scheduled batch and delivery
...
| `QA-PLT-063` | 2 of 4 racing writes answered 500 with no code: Internal Server Error |
...
| `QA-PLT-160` | a share capped at one read served 5 of ten simultaneous reads at worst across 5 rounds (served [1, 5, 5, 5, 5]). `open_share` reads `share['reads']` before the state check and writes `rea… |
...
## 8. Batch G — Documents: compile, dossier, pack, review, search
...
| `QA-PLT-169` | near-identical binary filed twice — as `application/pdf` and as `text/plain` — move coverage from 0.5 to 0.6667, because `text_indexed` is `media_type in TEXT_MEDIA` and the media type is… |
...
| `QA-PLT-173` | superseding an unread PDF with a 31-byte text note moved coverage from 0.5 to 0.625 (4 unread → 3) while the content it measured became unreachable: a term inside the superseded document … |
...
## 9. Batch K — Deletion, compaction and the operational tools
...
| `QA-PLT-093` | a raw UPDATE landed during a fold and the fold went on answering 'fold-afcb0433' while the table said 'renamed by a raw statement'. `_refuse_write_while_folding` is called from `Repositor… |
...
## 10. Batch L — Distributed evaluation, monitoring and parameter provenance
...
## 11. Batch J — Lifecycle and workflow correctness
...
| `QA-AM-138` | a finding under root 01a0a2d47400fe3eeb5cd2e6a0c6 was moved to root 01a0a2d474113bdfc61c97bc4888 by a call that never mentioned the first: `attach` writes `root_id` without asking whether… |
...
| `QA-FX-1208` | retiring 'prof-f2a64ed8' left version 1 live — retire marks the current version, so the previous one is uncovered and the profile goes on filling in defaults under a name the register rep… |
...
| `QA-GOV-282` | 'maya://model/dc-1b3bfd30' is retired and was accepted as the replacement: the check asks whether the URN is registered and never whether it is in service, so the successor chain points a… |
...
Five identity comparisons using `==` where the platform has `same_person`, all
closed and re-run green. Ten sites in `core/` now ask the question the one way:
break-glass authorisation and its post-hoc review, self-suspension,
recertification's reviewer and self-certification checks, the document
review's raiser-may-not-close and withdraw rules, retraining's
author-may-not-approve, and a saved view's owner. `MACHINE_IDENTITIES` was
added beside `same_person`, because `system` is the default actor on two
hundred service calls and a carve-out written per subsystem is one written
differently per subsystem — which is how a machine came to attest its own
output. And `AuthorityRegister.held_by` now matches with `same_person` rather
than filtering the table by the raw string, so a delegation recorded in the
prefixed form reaches the human who holds it.

---

## After the remaining eleven
