...
| `QA-GOV-132` | a figure attested as at a date a year away decides today's band: the amount reads 1000.0 and the band is 'bd-48461413'. `amount_for` takes the greatest `as_at` and nothing refuses one in … |
...
| `QA-PLT-063` | 6 of 24 racing writes across 6 rounds answered 500 with no code: Internal Server Error |
...
| `QA-PLT-093` | a raw UPDATE landed during a fold and the fold went on answering 'fold-fe3877f7' while the table said 'renamed by a raw statement'. `_refuse_write_while_folding` is called from `Repositor… |
...
| `QA-AM-138` | a finding under root 01a0a327d26a6a79af8206070813 was moved to root 01a0a327d27581741a4772f63f5e by a call that never mentioned the first: `attach` writes `root_id` without asking whether… |
...
**Batch D** closed the scope leak on both read-by-id routes.
`GET /findings/{id}` and `GET /documents/{id}` now load the subject BEFORE
authorising and pass its model, so the scope has something to apply itself to
— they were served whole to a reader the matching `?urn=` listing refused.
`GET /open-findings` says how many models scope removed and whether the total
is the estate's, because a silently short total gets reconciled against
somebody else's and the difference is read as a defect in the register. And
document search's `scoped` now reports whether the filter RAN rather than
whether an authoriser is wired.

The fifth, `QA-PLT-335`, turned out to be narrower than it read and is closed
by making the boundary explicit rather than by changing the check. An API key's
scope is applied in `authorise`, so a route that only authenticates narrows
nothing. All sixty-five parameterless GETs of that kind were driven as an
unprivileged principal against an estate holding a marked model and a marked
finding, and none returned either — they are closed vocabularies or they filter
by `visible()` themselves. `tests/test_scope_discipline.py` now fails the build
on a route added without being declared, which is the moment somebody is asked
whether it carries an estate.
