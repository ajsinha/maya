...
ID that means two things is an ID nobody can report a result against.
    ...
    python -m qa.regression_suite.verify              # assemble and check
    python -m qa.regression_suite.verify --check      # check only, write nothing
"""
...
def _generated() -> str:
    from qa.regression_suite.enumerate import build, render
    return render(build())
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="qa.regression_suite.verify")
