...
by a different method in `qa/regression_suite/refusals.py`, and the log says which method
...
from qa.regression_suite.enumerate import (_path_params, _required, operations, screens)
...
class Runner:
    ...
    def _call(self, method: str, path: str,
        ...
        if path in SESSION_ENDING:
            ...
            from qa.regression_suite.harness import sign_in
            response = client.request(method, path)
            sign_in(self.client)
            from qa.regression_suite.harness import carry_csrf
    ...
    def _cases(self, section_prefix: str):
        ...
        Taken from `qa.regression_suite.enumerate.build()` — the same function that
        ...
        """
        from qa.regression_suite.enumerate import build
