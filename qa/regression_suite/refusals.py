...
`qa/regression_suite/run.py` calls endpoints. Provoking `segregation_of_duties` means
...
def mapped() -> Dict[str, int]:
    from qa.regression_suite.enumerate import refusal_codes
...
def _scanned() -> List[pathlib.Path]:
    ...
    for folder in ("core", "routes", "db", "sdk", "tools", "qa"):
        base = ROOT / folder
        if base.is_dir():
    ...
    if rerun or not SEEN.is_file():
        ...
        subprocess.run(
            [sys.executable, "-m", "pytest", "-q", "-p", "no:randomly",
             "-p", "qa.regression_suite.codeprobe", "tests/"],
