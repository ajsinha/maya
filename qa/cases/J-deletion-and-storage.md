...
| QA-DEL-200 | `regression` | A deleted URN resolves to what happened to it | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-200` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-201 | `regression` | A tombstoned URN cannot be transitioned | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-201` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-202 | `regression` | A dry-run sweep reports and reclaims nothing | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-202` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-203 | `regression` | Delta is reported as out of reach, not skipped | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-203` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-204 | `regression` | Vacuum answers and reports what it reclaimed | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-204` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-300 | `regression` | The retention schedule is published with its reasons | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-300` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-301 | `regression` | A period is a floor and says so | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-301` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-310 | `regression` | A hold over a model that does not exist | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-310` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-311 | `regression` | A model-scoped hold with no identifier | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-311` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-312 | `regression` | A hold of a scope kind that is not one | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-312` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-313 | `regression` | Two holds with the same reference | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-313` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-314 | `regression` | Lift a hold with no reason | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-314` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-315 | `regression` | Lift a hold that does not exist | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-315` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-316 | `regression` | A hold has no end date, deliberately | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-316` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-317 | `regression` | A lifted hold no longer applies | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-317` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-330 | `regression` | The inference log says what it holds and for how long | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-330` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-331 | `regression` | Inference past its retention is reported, not hidden | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-331` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-4000 | `regression` | Every disposition is one of the three kinds | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-4000` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-4001 | `regression` | Every disposition says why | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-4001` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-4002 | `regression` | Every blocking disposition produces runnable SQL | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-4002` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-4003 | `regression` | No table is dispositioned twice | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-4003` | written while executing this section; the id is its own so a result is never credited to a case about something else |
| QA-DEL-4004 | `regression` | Every table carrying a model_id is dispositioned | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-4004` | written while executing this section; the id is its own so a result is never credited to a case about something else |
...
| QA-DEL-085 | `regression` | A sweep under an estate-wide legal hold | **reported by the suite** | `python -m qa.regression_suite.scenario_run --only QA-DEL-085` | written while executing this section; the id is its own so a result is never credited to a case about something else |
