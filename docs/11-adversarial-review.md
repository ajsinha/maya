...
| Laws-as-tests discipline | **Held in principle, and the build has tested the principle.** C-2, C-3 and H-6 would each have been caught earlier had the corresponding law existed. Since the review, seventeen more laws have been added: L-17 (contract–serving agreement), L-18 (no personal data in evidence nodes), **L-19** (composition is a monoid), and the fourteen warrant-admissibility laws L-W0…L-W13 — of which L-W8 caught a real error in a shipped example on the day it was written, and L-W11 and L-W13 each caught one in a shipped example on the day *they* were. The honest qualification is in [00 §12](00-mathematical-foundations.md#12-the-laws-maya-enforces): **fifteen of the twenty-one** foundational laws are executable today, up from seven of nineteen — L-1, L-2, L-3, L-9 and L-16 were closed in `tests/test_laws.py`, and writing them found two things a table could not: `baselined` was reachable by no declared edge (a second *initial* object rather than a violation, now stated), and `FRESHNESS` is not a semiring, since its zero does not annihilate. The remaining six are named there with the reason each is not yet executable, because a law that is stated but not executed did not prevent anything, and C-4's recurrence is what that costs |
