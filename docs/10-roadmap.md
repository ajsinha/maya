...
2. **Ship the hard architecture first, the pretty features later.** The evidence graph, the fibration and
   the hook protocol are load-bearing. Retrofitting immutability or extensibility is not possible; adding a
...
gantt
    ...
    section Phase 2
    Versions, features, hooks       :p2, after p1, 4M
    section Phase 3
...
| **Spikes** | PIT join at 1B rows; hook resolution p99 under load; sandbox escape testing; ONNX/PMML introspection breadth |
...
### Phase 2 — Versions, features and hooks (4 months)
...
| **Hook service**: resolution, signing, revocation, telemetry | FR-HOOK-001..012 |
| Hook flavours: `descriptor_only`, `python_sdk`, `rest_oip_v2`, `batch_spark` | FR-HOOK-002 |
| Upload wizard, feature reconciliation UI | — |

**Exit:** an execution engine runs a production model solely via a hook; an alias move switches the served
...
**Exit:** a monitoring breach automatically restricts a production hook; the overlay dashboard is used in
...
| Composite hooks | FR-HOOK-017 |
| Additional hook flavours: `sql_udf`, `stream`, `container`, `sheet` | FR-HOOK-002 |
...
| **Hook plane becomes a bank-wide SPOF** | L | **VH** | Independent scaling, regional failover, grace window, escrowed static descriptors, quarterly failure drills |
...
| Option | Assessment |
|---|---|
| **Buy a GRC/MRM platform** (OpenPages, SAS MRM, ValidMind, ModelOp) | Gets workflow and documentation quickly. Does not deliver artifact binding, feature management, hooks, or coverage of the quant and EUC estate. Would require a second and third system alongside. |
