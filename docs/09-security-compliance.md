...
|---|---|---|
| Enforcement point | One middleware in `run_maya_web.py`, not a check in each route. There are **117 mutating endpoints**, and a control 107 places have to remember will be missing from the 108th | Code (`core/authz/csrf.py`) |
...
| What may they do? | 8 roles over a closed vocabulary of **72 permissions** | 2.3 |
...
**72 permissions** in `resource:act` form. Two roles are supersets of others *by construction* rather
