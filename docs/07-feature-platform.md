...
| ~~**`L-17` does not run**~~ **It runs** | The law was blocked on the wrong thing. Contract–serving agreement compares what serving *must* read against what it *did* read, and the second half was thought to need a store to observe — but §7 of [04](04-architecture.md) says MAYA will not sit on the serving path, so the law had been made to depend on something the architecture forbids. **The engine attests** which namespaces it read and MAYA compares (`core/features/serving.py`). What a store would still buy is *observation* rather than attestation, which is a smaller and different claim. What follows was the state before that | C-2 was invisible to every design-time check, so this one has to run in production or not at all |
