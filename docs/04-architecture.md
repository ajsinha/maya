...
├── scheduler/       eighteen idempotent jobs, a runner and an optional in-process loop
...
| Async | Celery, APScheduler | `core/scheduler/`: **eighteen idempotent jobs** invoked by an ordinary authenticated call, so cron, a Kubernetes CronJob or a person produce identical results. An in-process loop exists and is off by default |
