...
Each claim is marked with where it is enforced. Sixteen of the twenty-one stated laws execute against
generated inputs; six do not, and are named with the reason. Three defects were found by writing the
executable form of laws that had been stated for months.
\end{abstract}
...
\item \Cref{sec:laws} states the discipline that keeps the rest from becoming ornament, gives the
      register of twenty-one laws with the five that do not execute named, and records three defects that
      only the executable form exposed.
...
(vii) the observation that the derive/declare boundary and the oracle boundary coincide
(\Cref{sec:automation}).
...
marker names a test, a module, or --- for six of the twenty-one laws --- says plainly that the claim is
...
\begin{remark}[An error this corrected]\label{rem:backwards}
The design note preceding the implementation said that a refinement is ``at a type no wider''. That is
backwards: standing in for something requires accepting \emph{at least} what it accepted, so a wider
acceptance is admissible and a narrower one regresses. The prose came from the intuition that a subtype
is narrower --- true of the values a type denotes, false of the inputs a slot accepts --- while the code's
acceptance relation had it right. Writing the implementation found the error in the note, which is the
ordinary direction of that traffic and is recorded here because a paper that reports only its successes
misrepresents how the results were obtained.
\end{remark}
...
The first three lines were previously three implementations of one relation; they are now one function,
which is checkable by inspection and is checked by inspection. The fourth line was not previously askable
at all. It is the meet doing work: a featureset that must serve two models must refine their meet, and
when the meet does not exist, the honest answer is that no featureset can, with the offending slot named.
That the question is now expressible is a consequence of \Cref{thm:lattice}; that nothing yet asks it is a
consequence of nobody having needed to, and is recorded rather than presented as a capability in use.
...
\end{proof}
\stated{there is no aggregate $\rho$ in the reference implementation, so \Cref{thm:noncomp} is a theorem
the platform respects by not claiming its negation. \Cref{prop:compose} supplies the derived composite
signature such a $\rho$ would quantify over, and \texttt{shared\_dependencies} computes the obstruction;
the functor itself is unbuilt. This is law \textsf{L-14}, and \Cref{sec:laws} lists it among the six.}
...
\subsection{What writing the operator down found}\label{sec:operatorfound}

Two defects were exposed by stating \Cref{prop:asof} as executable properties rather than as a
description. Both had been present for months, both are of a kind that is invisible to ordinary testing,
and both are worth reporting because they are characteristic.

\paragraph{The published rule was not the applied rule.} The platform publishes the point-in-time
predicate as a string, in every feature-set plan and every execution warrant, because the engines that
implement it are not the ones this system runs. The published string bounded ingest by the observation
time alone --- the rule from before $\min$ was introduced. An engine implementing the published contract
faithfully therefore admitted rows the platform's own assembly refuses, and the disagreement would have
surfaced as an unreproducible training set with two correct-looking implementations and no way to tell
which was wrong. The test that now guards this does not compare strings; it \emph{evaluates} the published
predicate against the same records and demands the same answer as the operator.

\paragraph{The independent verifier disagreed with the thing it verifies.} Assembly is checked by a second
computation that deliberately does not reuse the assembly path, so that agreement means something. That
second path bounded ingest by the observation time alone, while the operator bounds it by $\min(\ell,a)$.
The two therefore disagreed whenever a set was assembled after its labels matured --- which is the
ordinary case --- and the verifier reported a mismatch on a \emph{correct} assembly. A false positive in
the control that verifies a control is worse than no control, because it teaches whoever reads the report
to discount it.
...
\emph{What does this rest on?} The free variables. This is distinct from the transitive ancestor set, and
writing the tests made the distinction precise rather than leaving it to be discovered: the ancestor walk
returns every ancestor including derived ones, which is the right answer to \emph{what breaks if this
changes}; the free variables are the base features and nothing else, which is the right answer to
\emph{what data does this ultimately read}. The second is the first minus its derived members, and that is
...
The reference implementation states twenty-one laws. Fifteen execute; six do not.
...
\textsf{L-14} & lax monoidality of risk (\Cref{cor:lax}) & \emph{not built} \\
\textsf{L-15} & fibration completeness: no empty fibre & \emph{not built} \\
\textsf{L-16} & no obligation contradiction & runs, and enforces \\
\textsf{L-17} & contract--serving agreement & \emph{not built} \\
...
The six that do not run are named with their reasons, in the test file as well as in the table, because a
...
no implementation; no consistency radius is computed anywhere. \textsf{L-14} needs composite warrants and an
aggregate risk function; \Cref{prop:compose} now gives it a derived signature to quantify over, but the
functor is design. \textsf{L-15} needs a loader that refuses to boot on a partial fibre; artefact classes are
strings today. \textsf{L-17} needs an online serving store to compare against; half of it exists, in that the
system computes what serving \emph{must} read.
...
\subsection{What executing them found}

Three defects were found by writing the executable form of laws that had been stated for months, and each
is characteristic of a different failure mode.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \emph{A design note that was backwards} (\Cref{rem:backwards}). The prose said refinement was at a type
      no wider; the code's acceptance relation had it right. Stating the order found the disagreement.
\item \emph{A published contract that did not match the implementation} (\Cref{sec:operatorfound}). The
      point-in-time predicate handed to external engines was the rule from before the $\min$ was introduced.
      The old test asserted a substring of it, which is a check loose enough to pass on the wrong rule; the
      new one executes the published predicate and demands the operator's answer.
\item \emph{An independent verifier that disagreed with the thing it verified}
      (\Cref{sec:operatorfound}). The second computation bounded the ingest clock differently from the first,
      so it reported a mismatch on correct assemblies.
\end{enumerate}
...
\paragraph{Six laws do not execute.} They are listed in \Cref{sec:laws} with reasons. In particular
...
Six of twenty-one laws do not execute. And some facts --- the tiering rule, the regime encoding, the
