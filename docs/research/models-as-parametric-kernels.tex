...
\item \Cref{sec:laws} states the discipline that keeps the rest from becoming ornament, gives the
      register of twenty-one laws with the five that do not execute named, and states the properties an
...
The reference implementation states twenty-one laws. Sixteen execute; five do not.
...
The five that do not run are named with their reasons, in the test file as well as in the table, because a
...
aggregate risk function; \Cref{prop:compose} gives it a derived signature to quantify over, but the functor
is design. \textsf{L-17} needs an online serving store to compare against; half of it exists, in that the
...
\paragraph{Five laws do not execute.} They are listed in \Cref{sec:laws} with reasons. In particular
