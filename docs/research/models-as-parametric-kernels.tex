...
generated inputs; three do not, and are named with the reason in the place a reader would look for the
...
The account has a reference implementation, and this revision names it. \textsc{Maya} is an open
governance register of some 318 modules and 5{,}394 tests --- \url{https://github.com/ajsinha/maya} ---
in which each of the four derivations is a
module rather than a proposal, the twenty-one laws are a test file that fails a build, and the three that
do not execute are listed in that file with their reasons. \Cref{sec:implementation} gives the numbers,
the map from proposition to module, and --- more usefully --- the four places where building the thing
...
2026-13 \cite{sr262}. A reader should treat the second differently from the first. It is recent, its
status and final text may not yet be settled in the reader's jurisdiction or at the reader's date, and an
earlier draft of this paper narrated it as the settled successor to SR~11-7 \cite{sr117} without saying
so. That was an overreach and it is corrected here.
...
independent-challenge requirement and the inventory obligation since 2011. And the definitional narrowing
we use to motivate plural scope is a \emph{recurring} feature of model-risk definitions rather than a
property of one revision --- SR~11-7's own ``quantitative method \ldots that applies statistical,
economic, financial, or mathematical theories'' already sits awkwardly against a deterministic rule set
that a firm nonetheless has to govern, and against a language model that governs nothing and drafts
everything.
...
marker names a test, a module, or --- for three of the twenty-one laws --- says plainly that the claim is
stated and not executed. We regard the second kind of marker as the more important one, for the reason
\Cref{sec:laws} gives: a document implying that every law is checked, while three are not, is the exact
failure that section exists to prevent.

We should record that an earlier draft of this paper failed that test about itself. It gave the number of
non-executing laws as five in the abstract, five here and six three lines below, while the table in
\Cref{sec:laws} and the test file both said three. A reviewer did not catch it; a count check over the
paper now does, and it runs beside the law suite. We leave the episode in because the alternative --- a
silent correction --- is precisely the class of repair this paper argues against, and because a foundation
whose own document drifts from its own tests is making the argument for the discipline more effectively
than the section that states it.
...
An earlier draft described a ``reference implementation'' and named neither it nor its size, and a reviewer
read the paper as having none. That was a fair reading of what was written, and this section is the repair.

\subsection{What exists}

\textsc{Maya} is an open model and AI lifecycle assurance register, at
\url{https://github.com/ajsinha/maya}. It is not a companion artefact built to illustrate the paper; the paper
is the account of what building it required. At the revision this text describes it is 318 Python modules
that type-check cleanly, 82 tables in one typed schema from which both dialects' DDL is generated, 494
locked HTTP paths, and 5{,}394 tests that run on every push.

Each of the four derivations is a module rather than a proposal.

\begin{center}
\small
\begin{tabular}{@{}p{0.30\linewidth}p{0.30\linewidth}p{0.34\linewidth}@{}}
\toprule
\textbf{Proposition} & \textbf{Module} & \textbf{What it refuses} \\
\midrule
\Cref{prop:classification}, trainability from the inhabitation of $P$ &
\texttt{core/fibres/}, \texttt{core/domain/algebra.py} &
a declared class. The class is computed at registration and a version whose declaration disagrees is
refused, not corrected \\
\Cref{prop:onerelation}, one schema order &
\texttt{core/features/composition.py}, \texttt{core/registry/composition.py} &
an alias move whose contract narrows an output, and a featureset slot whose candidates have no meet ---
naming the slot \\
\Cref{prop:asof}, the point-in-time read as an operator &
\texttt{core/features/pit.py} &
a training assembly reading $a$ rather than $\min(\ell,a)$; the bound is applied in one place and the
saturation law is a property test over it \\
\Cref{prop:universal}, provenance over $\Nat[X]$ &
\texttt{core/evidence/semirings.py}, \texttt{core/evidence/engine.py} &
nothing directly --- it is the traversal six governance questions are pushforwards along \\
\bottomrule
\end{tabular}
\end{center}

The twenty-one laws are \texttt{tests/test\_laws.py}. Eighteen execute against generated inputs; three do
not and are listed \emph{in that file} with their reasons, and a test asserts that the list of
non-executing laws in the file matches the set of laws with no runner. That is the arrangement
\Cref{sec:laws} argues for, and it is why the count defect this revision corrects was a defect of the
paper and not of the system.

\subsection{Four things building it changed}

The more useful report is not that the propositions are implemented but that four of them came back
altered, and in each case the alteration is in the paper above.

\paragraph{The $\min$ was wrong first.} \Cref{cor:repro} says the ingest bound is $\min(\ell,a)$. The
implementation read $a$, which is the natural thing to write and produces a training assembly that is
correct on the day and different a year later. It was not found by review; it was found by the saturation
property test, which is the argument of \Cref{sec:laws} in one line.

\paragraph{The meet had no caller for a year.} \Cref{prop:nomeet} presents the partiality of the meet as
an informative refusal. In the implementation the order is called from three production paths and the meet
from the law suite only. \Cref{sec:limits} records this, and it is worth restating here because it is the
commonest way a formalisation flatters itself: an operation that exists is not an operation in use.

\paragraph{The fibration forced its own base to be computed.} \Cref{prop:derivedbase} is the one result in
this paper that we did not go looking for. It arrived as a start-up failure --- a class with no fibre ---
and the choice was to make the family partial or to compute the index. The mathematics is unremarkable;
that the constraint is invisible in the mathematics and decisive in the application is the paper's own
thesis applied to its own machinery.

\paragraph{The interaction premium is still not computed.} \Cref{thm:noncomp} and \Cref{cor:lax} say what
a correct aggregate cannot be. The implementation now composes a \emph{chain} and authorises it as one
unit, and its tier is the \emph{join} of its nodes' --- which is exactly what the theorem permits and no
more. What composes is the order, not the number. A composite warrant can say \emph{this chain is at least
tier 1 and every link resolves}; it cannot say \emph{this chain is 0.83 risky}, and the reason it cannot is
the theorem rather than a backlog item. We regard this as the honest state and not as a partial result.

\paragraph{A fifth alteration: the boundary needed a type, not a caveat.} None of the four propositions
says anything about what happens where the register stops --- where it must rely on a party it does not
control. In the implementation there turned out to be five such places: an external timestamping
authority over the evidence chain, a third-party extension package, an export from another model
registry, a sweep run by somebody else's scanner, and an evidence pack handed to a supervisor who has no
account. Each one presented the same failure mode, and it is a failure mode this paper's apparatus does
not by itself prevent: at a boundary, the natural report is the answer one wishes one had, and the
overstatement is invisible precisely because nobody can see past the boundary to check it.

The corrective in each case was to widen a type rather than to add a warning. The clearest instance is
the timestamp. A held \textsc{rfc}~3161 token that nothing has verified is not the same object as a
verified one, and it is not the same object as no token; collapsing it into either --- and both collapses
are convenient --- produces a system that reports either a verification it did not perform or an absence
of evidence it in fact holds. So the state is three-valued and the third value is inhabited. The
analogous moves elsewhere: a connector produces \emph{candidates} rather than registrations, because the
five governance facts that make a registration meaningful appear in no model registry and inferring them
would manufacture exactly what the register exists to hold; a discovery sweep is admitted whole or
refused whole, because a partial admission yields a precision figure that grades the subset the register
chose to keep rather than the scanner; and an installed extension and an enabled one are distinct states,
because a control that is switched on by a dependency resolution is a control nobody turned on.

We record this because it is the one place where the implementation demanded something the theory does
not supply. The propositions constrain what the register may \emph{conclude} from what it holds; they say
nothing about the epistemic status of what it holds, and at a boundary that status is exactly what is at
issue. A discipline of making the unknown state \emph{representable} --- rather than reporting it as its
nearest convenient neighbour --- is not a consequence of anything proved here. It is an obligation the
practice added.

\subsection{Two case studies that were not designed to fit}

The objection above applies to the worked examples as much as to the modules: fifteen demonstrations
written by the author of the platform establish that the thing is self-consistent and very little else. A
sixteenth built the same way would not help.

So two of them are taken from the public record instead, and they were chosen to fail in different
places, because one outside scenario is an anecdote.

\paragraph{The first: a model change nobody was made to justify.} It reconstructs the model governance failure at the
centre of the 2012 JPMorgan Chief Investment Office losses, as documented by the firm's own Management
Task Force report and by the United States Senate Permanent Subcommittee on Investigations
\cite{jpmtaskforce2013,psi2013}. The sequence is theirs and the controls are the platform's; nothing about
the shape of the failure was selected. The figures in the script are a reconstruction and are labelled as
one --- what is being tested is a shape of failure, not a balance sheet.

Briefly: a synthetic credit portfolio breached its VaR limit; a new VaR model went into production days
later, reported VaR fell by roughly half and the breach disappeared; the model had been approved subject
to further work that was not completed; it was implemented as a chain of spreadsheets with manual
transfers between them, one of which divided by the \emph{sum} of two rates where it should have divided
by their \emph{average}.

Six governance questions arise. The register answers five: whether the replacement is a material change
(computed from the two versions, and material because a guarantee loosened); whether the alias may move
without revalidation (refused); whether swapping the model closes the breach the old model reported (it
does not, and the finding survives and keeps blocking); whether ``approved subject to further work''
binds anything (it does, and it expires); and whether a spreadsheet reimplementation agrees with the
model it reimplements (answerable by the recode harness, which reports the \emph{shape} of the
disagreement rather than a pass rate). The sixth --- whether the arithmetic inside the spreadsheet cell
is correct --- it cannot see, and does not claim to.

The third of those is the one worth dwelling on. A limit breach is a fact about the \emph{portfolio}, and
a model reporting a smaller number has not made the portfolio safer. The design closes a \emph{breach}
when the monitor recovers and deliberately leaves the \emph{finding} open; here the model did not recover,
it was replaced, and the finding is still open and still blocking.

\paragraph{A seventh question, which the script did not anticipate.} The demonstration was written to show
the six above. It was refused something else: an \emph{attested} model record cannot acquire a new version
at all. Somebody must open an amendment, with a name on it and a stated scope, and the act goes on the
chain. In the failure being reconstructed, the new model entered production days after the breach and the
question of what was being amended was never put.

That is a stronger control than the one the case study set out to exhibit, and it surfaced only because
the scenario was not ours. It is the clearest single argument for the exercise, and it is also the shape
of evidence this section can offer in place of the deployment study it cannot.

\paragraph{And the closing section lists what the platform does not reach.} Four of the seven links in
that failure chain are outside it: the arithmetic inside the cell, the manual transfer between
spreadsheets, the trader marks, and whether anybody read the finding. A demonstration that stopped before
that list would be marketing. Running the case study also found two defects in the platform --- one of
them a sentence reporting a five-month overrun against a regulatory commitment in language that read as a
near miss, and a test that had been passing on it.

\paragraph{The second: a fact the register can see and cannot act on.} The fifteenth case study leaves
financial services, which is the only way to test whether controls argued from banking supervision
transfer at all. It reconstructs the Dutch childcare benefits affair, in which a self-learning risk
classification model selected benefit claims for manual review and nationality was among its indicators;
roughly twenty-six thousand families were wrongly treated as fraudulent, the data protection authority
found the processing unlawful and discriminatory, and a parliamentary inquiry was followed by the
government's resignation \cite{ap2020nationality,ongekendonrecht2020}. The figures are a reconstruction
and no applicant data is used.

Here the register answers a narrower set of questions and one of its answers is uncomfortable. It
\emph{knows} the model reads a protected characteristic, computed by walking the feature contract to the
pinned view versions rather than taken on anybody's declaration, and it knows separately that the model
reads a known proxy for one. It reports that deleting the protected column is not a remedy --- in the
reconstruction it removes about two-thirds of the disparity in selection and leaves the rest in postcode.
What it will not do is refuse on the tag: a rule prohibiting direct use while permitting controlled use
for fairness testing is not a sentence a platform can evaluate on an institution's behalf, and one
refusing on the tag alone would refuse the fairness testing the same regulation requires. So the facts are
published at the approval and alias-move gates, the rule stays the institution's, and the estate view
reports \emph{whether any rule reads them} --- which is the part a vocabulary-and-nothing-else would leave
out.

\paragraph{And it found a limitation rather than a control.} Where the fourteenth surfaced a refusal its
author had not anticipated, the fifteenth surfaced a gap. \texttt{policy\_decision} is the highest purpose
class the taxonomy has, and the model still tiers below the top band, because the other axis of the
tiering lattice is exposure denominated in currency. A model that ruins twenty-six thousand households
without moving a balance sheet therefore sits beneath a mid-sized pricing model. The lattice was built for
an estate where consequence is money; the case study is outside that estate, and the mismatch is a
property of the construction rather than of the implementation. It is recorded rather than repaired,
because repairing it by adding a second unit of account would be a design decision this paper has not
argued for.

The closing section of that case study is longer than the platform's contribution to it, and deliberately:
the recovery policy that turned a selection into a debt was policy rather than a model, the blacklist was
a list, whether anybody read the finding is a management act, and the harm is not a governance artefact.
Most of that failure is outside what any register holds, and a demonstration that did not say so would be
the thing this section exists to avoid.

\subsection{What the implementation does not establish}

It shows the derivations are computable, that they refuse real inputs, and that the laws can be executed
rather than asserted. It does not show that a system built this way is easier to construct, extend or
operate than one built without it. There is no comparative study, no user evaluation, and no deployment at
a supervised institution. \Cref{sec:limits} states this as the principal missing evidence and the
implementation does not change it --- a large test suite is evidence that a thing is internally consistent,
not that it is useful.

Two further cautions. The system is written by the author of the paper, so it is a demonstration of
sufficiency and not an independent replication. And its refusals are calibrated against a fictional but
realistic estate rather than a real one; whether a refusal naming a slot with no meet is actionable to a
model owner under time pressure is the fourth falsification criterion in \Cref{sec:limits}, and it remains
open.
...
usually serves neither. The implementation carries a second register of the same content written for the
second audience: twenty help pages and eight walkthroughs that state each derivation in operational terms
with no notation, and one --- \emph{What \textsc{Maya} refuses to do} --- that is the boundary of
\Cref{sec:automation} written as fourteen refusals and what each protects. A reader who wants the argument and
not the algebra should start there.
...
an artefact rather than recording it.
...
is substantial and is not evidence of the claim that matters. A large test suite shows a thing is internally
...
Three of twenty-one laws do not execute. And some facts --- the tiering rule, the regime encoding, the
...
\bibitem{syri2020} Rechtbank Den Haag. \emph{NJCM c.s.\ v.\ de Staat der Nederlanden} (SyRI).
5 February 2020, ECLI:NL:RBDHA:2020:865.
