...
*Last updated after milestone 30. This section is the authoritative record of what
...
- **Parameters are stored, not computed.** MAYA records what a fit returned and
  refuses it unless a warrant it issued produced it, but nothing estimates,
  trains or calibrates: that happens in an execution engine, and a captive one
  for QuantLib is still unbuilt.
...
- **The captive engine implements four runtimes of seventeen.** No container, no
