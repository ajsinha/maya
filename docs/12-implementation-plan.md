...
**Over 5,300 tests**, plus a scale suite excluded by default. **18 of 21** foundational
...
| ✅ | **Content system** (`core/content/`) | **Complete** | Help and about pages are markdown under `content/`, rendered server-side and cached on modification time. 20 help topics in 7 sections (~69,000 words) plus a competitive analysis on About. Versioned and reviewable in a pull request alongside the behaviour they describe |
...
| ✅ | **Java SDK** (`sdk/java`) | **Complete** | Release 17, **no runtime dependencies** — `java.net.http` and two hundred lines of `Json.java`, for the same air-gap reason every asset here is vendored. The README was the contract before there was an implementation and is kept in that order. A `POST` is never retried and the timeout exception says so; an outage throws `Unreachable`, which deliberately does **not** extend `Refused`, because a client that collapsed them treats an outage as a governance verdict. `MayaTest.DecidesNothing` walks the source, strips the comments and fails on a trainability class or a local `isApproved` in the **code** |
...
seven CI gates that actually fire.
...
| | Gate | State |
|---|---|---|
| 1 | Lint, format, type check (`ruff`, `mypy`) | **Runs** — `ruff check .` with a rule set chosen in `pyproject.toml` rather than inherited, and `tools/ci/typecheck.py`, which gates on the 336 modules that check cleanly and carries the other 61 in `mypy_backlog.txt`. Not `--strict`: adopting it across 397 modules in one release produces a blanket ignore, which is the same thing as `mypy \|\| true` wearing a hat |
