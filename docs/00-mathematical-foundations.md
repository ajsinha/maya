...
| **L-20** | *Schemas are a lattice.* `A ⊑ B` ("A can stand in for B") is a partial order; meet and join exist on every finite fragment; the empty schema is top. | §6.3 | **Executable and enforcing.** `core/domain/lattice.py`; `tests/test_laws.py::TestL20SchemasFormALattice`. Written because **four** places asked one question — `substitutable` (L-12), a slot loop (L-W10), `refines` for contracts, and nothing for `input_to` — and four implementations of one order disagree eventually, in the direction of permitting more. L-12 and L-W10 now go through it. Meet is **partial**, and informatively so: two schemas whose shared slot has two types have no meet, which is the honest answer to *can one featureset serve both models* |
| **L-21** | *Composition type-checks.* A `input_to` edge holds only if what the source produces can stand in for what the target reads. | §5.2 | **Executable and enforcing.** `core/registry/composition.py`; `tests/test_laws.py::TestL21FeedsIsCompositionRatherThanADrawing`. Until this, `input_to` was recorded and never checked — a blast radius over edges nobody had validated. With it, a composite has a **derived** schema (`composite_schema`), which is what `L-14` needs to quantify over |
