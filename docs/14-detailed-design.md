...
| Everything else | **Seven of the eight are built** — `ETag`/`If-None-Match`/`If-Match` and `Idempotency-Key` in `core/concurrency/`, `?fields=`, keyset cursors and `Sunset` in `core/http/conventions.py`, `?as_of=` in `core/registry/asat.py`, SSE on the live log. **`?expand=` is not**, and `/derivations/{id}` is a helper rather than a route. `GET /api/v1/conventions` publishes what each promises, so a client does not hold its own idea of how paging works |
...
the 337 modules that pass and carries 61 in a backlog file, because `mypy || true` is a step that
always passes — the defect this codebase is named for — and `--strict` across 398 modules in one
...
| ~~**Composite warrants**~~ **built**; the interaction premium **refused** | A chain resolves as one unit or is refused as one, and its tier is the **join** of its nodes' — exactly what `L-14` permits and no more. The aggregate `ρ` is not coming: a network that *copies* a dependency and one that *duplicates* it produce identical component ratings, so any single figure over them is blind to precisely what it would exist to find. What composes is the **order**, and that is a better answer than a number a committee cannot decompose |
