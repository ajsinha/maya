...
That is the whole design rule, and it is why the schema is forty-six tables rather than two hundred.
...
column and what must be a pointer. §4 is the forty-six tables, grouped by the question each group
...
populated database is a manual `ALTER` in both dialects, which is acceptable at forty-six tables and
