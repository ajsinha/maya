...
Every scenario in `tools/qa/scenarios/` is an executable statement about a
...
seconds each; `python -m tools.qa.scenario_run` is how they run). It asserts
...
class TestTheHarnessStillRefusesToLie:
    def test_it_signs_in_through_the_form(self):
        ...
        being opened."""
        source = (ROOT / "tools" / "qa" / "harness.py").read_text(
            encoding="utf-8")
